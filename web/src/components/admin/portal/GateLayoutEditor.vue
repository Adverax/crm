...
import type { PortalViewField, GateBodyStep } from '@/types/portals'

const props = defineProps<{
  fields: PortalViewField[]
  bodySteps: GateBodyStep[]
}>()
...
const soqlStepNames = computed(() =>
  props.bodySteps.filter((s) => s.type === 'soql' && s.name).map((s) => s.name),
)
...
<template>
  <div class="flex gap-4 min-h-[400px]" data-testid="layout-fields-editor">
      ...
      <div v-else class="divide-y">
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <div class="min-w-0">
              <div class="text-sm font-medium font-mono truncate">{{ field.name || 'unnamed' }}</div>
            </div>
            <Badge
    ...
    <div class="flex-1 min-w-0">
      ...
      <div v-else class="space-y-4">
        ...
        <div class="space-y-1">
          ...
          <Input v-model="selectedField.name" placeholder="field_api_name" class="font-mono" data-testid="field-name-input" />
        </div>

        <div class="space-y-1">
          <Label class="text-xs">Expression (CEL)</Label>
          <p class="text-[11px] text-muted-foreground">
            Data source. Can reference SOQL steps (<span class="font-mono">{{ soqlStepNames.join(', ') || 'none' }}</span>) and other fields.
          </p>
          <ExpressionBuilder
            :model-value="selectedField.expr ?? ''"
            context="default_expr"
            height="120px"
            placeholder="data.Name + ' (' + data.Industry + ')'"
            :show-field-picker="false"
            data-testid="field-expr-input"
        ...
        <div class="space-y-1">
          ...
          <ExpressionBuilder
            ...
            context="gate_when"
            height="80px"
            placeholder="has(data.amount)"
            :show-field-picker="false"
            data-testid="field-when-input"
