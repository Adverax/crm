/**
 * CEL Expression Editor — Smart Autocomplete for CodeMirror 6
 *
 * Context-aware autocompletion for CEL expressions:
 * - After "record." → record field completions
 * - After "old." → old value completions (validation_rule/when_expression only)
 * - After "args." → dynamic hint (portal_when/gate_when)
 * - After "datasets." → dynamic hint (gate_when)
 * - After "data." → dynamic hint (gate_when)
 * - After "user." → user property completions (id, profile_id, role_id)
 * - After "fn." → custom function completions with signatures
 * - General → context variables, fields, functions, keywords, operators
 *
 * Dependencies:
 *   npm install @codemirror/autocomplete @codemirror/state
 *
 * Usage:
 *   import { celAutocomplete } from './cel-autocomplete'
 *   const ext = celAutocomplete({ fields, params, functions, context: 'validation_rule' })
 *   // Add `ext` to CodeMirror extensions array
 */
...
// --- Types (inline for standalone usage; replace with imports if using types.ts) ---
...
function buildFieldCompletions(prefix: string, fields: FieldInfo[]): Completion[] {
  return fields.map((f) => ({
...
/**
 * Creates a CodeMirror autocompletion extension for CEL expressions.
 *
 * @param config - Autocomplete configuration with fields, params, functions, and context
 * @returns CodeMirror Extension to add to the editor
 *
 * @example
 * ```ts
 * const ext = celAutocomplete({
 *   fields: [{ apiName: 'FirstName', label: 'First Name', fieldType: 'text' }],
 *   functions: [{ name: 'greet', params: [{ name: 'name' }], returnType: 'string' }],
