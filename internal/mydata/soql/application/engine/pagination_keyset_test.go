...
func TestKeysetFieldsInSelect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check for Id in SQL
			hasId := strings.Contains(compiled.SQL, "record_id")
			if tt.wantId && !hasId {
				t.Errorf("SQL should contain record_id: %s", compiled.SQL)
			}

			// Check for duplicates in SELECT clause only
			// record_id can appear multiple times (SELECT and ORDER BY), but not twice in SELECT
			selectEnd := strings.Index(compiled.SQL, "FROM")
			if selectEnd > 0 {
				selectPart := compiled.SQL[:selectEnd]
				count := strings.Count(selectPart, "record_id")
				if count > 1 && !tt.duplicates {
					t.Errorf("SELECT should not have duplicate record_id (found %d): %s", count, selectPart)
				}
			}
		})
	}
